...
from src.collator_stage2 import split_vlm_inputs, get_dense_rep, split_and_process_vlm_inputs
from src.model_stage2 import MMEBModel
...
class GradCacheLateProcessTrainer(MMEBTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        insert_eos = False
        eos_token_id = ""
        if self.model_args and hasattr(self.model_args, 'use_eos_contrastive'):
            insert_eos = self.model_args.use_eos_contrastive
            if hasattr(self.model_args, 'eos_token_id'):
                eos_token_id = self.model_args.eos_token_id
        
        if insert_eos:
            from src.collator_stage2 import create_process_fn_with_eos
            process_fn = create_process_fn_with_eos(
                processor=self.processing_class,
                backbone_name=self.args.model_backbone,
                max_length=self.max_length,
                insert_eos=True,
                eos_token_id=eos_token_id
            )
            logger.info(f"✓ Trainer: Auto-insert EOS enabled (token_id={eos_token_id})")
        else:
            process_fn = functools.partial(
                process_vlm_inputs_fns[self.args.model_backbone], 
                processor=self.processing_class, 
                max_length=self.max_length
            )
            logger.info("✓ Trainer: Standard process function (no EOS insertion)")
        self.gc = GradCache(
            models=[self.model, self.model],
            chunk_sizes=[self.args.gc_q_chunk_size, self.args.gc_p_chunk_size],
            loss_fn=loss_fn,
    ...
    def training_step(self, model, inputs, *args, **kwargs) -> torch.Tensor:
        ...
        _distributed = self.args.local_rank > -1
        self.gc.models = [model, model]
        loss_output = self.gc(queries, targets, no_sync_except_last=_distributed)
        ...
        if isinstance(loss_output, dict):
            ...
            if hasattr(self, 'state') and self.state is not None:
                if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
                    metrics = {
                        'train/pooled_loss': loss_output['pooled_loss'].item(),
                        'train/eos_loss': loss_output['eos_loss'].item(),
            ...
            return total_loss / self._dist_loss_scale_factor
        else:
            return loss_output / self._dist_loss_scale_factor
