...
from src.model_utils import (
    LLAVA_NEXT, QWEN2_VL, PHI3V, get_backbone_name, print_master, 
    QWEN2_5_VL, backbone2model, backbone2model_bidirectional
)
...
class MMEBModel(nn.Module):
    ...
    def load(cls, model_args: ModelArguments, is_trainable=False, **kwargs):
        ...
        is_stage1_checkpoint = False
        use_bidirectional = False
        if os.path.exists(os.path.join(checkpoint_path, 'stage1_info.pt')):
            is_stage1_checkpoint = True
        ...
        if model_args.model_backbone in {LLAVA_NEXT, QWEN2_VL, QWEN2_5_VL}:
            ...
            if use_bidirectional:
                model_class = backbone2model_bidirectional[model_args.model_backbone]
                print_master(f"→ Using BIDIRECTIONAL model class: {model_class.__name__}")
            else:
                model_class = backbone2model[model_args.model_backbone]
                print_master(f"→ Using standard model class: {model_class.__name__}")
        ...
        if use_bidirectional:
            print_master("\n→ Verifying bidirectional model loaded correctly...")
            cls._verify_bidirectional_model(model)
